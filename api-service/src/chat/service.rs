...
pub struct ChatService(Arc<RwLock<HashMap<i64, HashSet<Sender>>>>);
...
impl ChatService {
    ...
    pub async fn user_connected(self, id: i64, websocket: WebSocket) {
        ...
        while let Some(result) = user_ws_rx.next().await {
            ...
            match msg.to_str() {
                Ok(msg) => {
                    println!("{}: {}", id, msg);
                    if let Err(err) = self.handle_message(msg, id).await {
        ...
        self.remove_tx(id, tx_id).await;
        if let Err(err) = agent.await {
            log::warn!("join agent task 錯誤：{}", err);
        }
    }

    async fn handle_message(&self, msg: &str, id: i64) -> Fallible<()> {
